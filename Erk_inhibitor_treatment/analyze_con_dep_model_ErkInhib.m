% This function loads the data generated by con_dep_model_ErkInhib.m scripts for xx = 0:100
% and determines the time it takes for the next wave to form following Erk inhibitor treatment

clear data_store
data_store = cell(101,1);
for i = 0:100
    if (exist(sprintf('con_dep_ErkInhib_%d.txt',i), 'file') == 2)
        data_store{i+1} = importdata(sprintf('con_dep_ErkInhib_%d.txt',i));
    end
    i
end
...
legend off
standardizePlot(gcf,gca,strcat('con_treatment_vs_nextwave'));
close(fig);
